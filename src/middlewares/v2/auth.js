...
exports.protect = asyncHandler(async (req, res, next) => {
    ...
    if (currentUser.deactivationStatus || currentUser.deleted) {
        return next(
            new AppError(
                "Account deactivated or deleted, please contact support",
                403
            )
        );
    }
    ...
    if (currentUser.blocked?.status) {
        ...
        if (
            currentUser.blocked.unblockDate &&
            new Date() > currentUser.blocked.unblockDate
        ) {
            ...
            let message = `Your account has been blocked. Reason: ${
                currentUser.blocked.reason || "Not specified"
            }`;
...
const commonConstraintsSchema = Joi.object({
    ...
    email: Joi.string().email().required(),
    password: Joi.string().min(6).required(),
    pushToken: Joi.string().min(2).required(),
